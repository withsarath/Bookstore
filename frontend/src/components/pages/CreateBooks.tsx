...
const CreateBooks: React.FC = () => {
  ...
  const handleSaveBook = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await API.post<Book>('/books', data);
      ...
      setLoading(false);
      const message = axios.isAxiosError(error) ? error.response?.data?.message : 'Error creating book';
    ...
    <div className='max-w-4xl mx-auto pb-12 px-4'>
      ...
      <div className="bg-white dark:bg-slate-800 p-8 md:p-12 rounded-[2.5rem] shadow-2xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-700 animate-in slide-in-from-bottom-8 duration-500">
        <div className="mb-12 text-center md:text-left">
          <h1 className='text-4xl font-black text-slate-900 dark:text-white mb-3 tracking-tight'>Add New Book ✏️</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium text-lg leading-relaxed">Fill in the details below to expand your collection</p>
        </div>

        {loading && <div className="flex justify-center mb-8"><Spinner /></div>}

        <form onSubmit={handleSaveBook} className="space-y-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div>
                <label className='block text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3 ml-1'>Total Pages (optional)</label>
                <input
                  type='number'
                  value={totalPages}
